...
/obj/item/dyespray/proc/dye(mob/target, mob/user)
	...
	if(!gradient_or_hair || !user.canUseTopic(src, be_close = TRUE, no_dexterity = TRUE, FALSE))
		return
	...
	if(gradient_or_hair == DYE_OPTION_HAIR_COLOR)
		...
		if(!new_color || !user.canUseTopic(src, be_close = TRUE, no_dexterity = TRUE, FALSE))
			return
		...
		if(!beard_or_hair || !user.canUseTopic(src, be_close = TRUE, no_dexterity = TRUE, FALSE))
			return
		...
		if(!new_grad_style || !user.canUseTopic(src, be_close = TRUE, no_dexterity = TRUE, FALSE))
			return

		var/new_grad_color = input(usr, "Choose a secondary hair color:", "Dye Spray", human_target.grad_color) as color|null
		if(!new_grad_color || !user.canUseTopic(src, be_close = TRUE, no_dexterity = TRUE, FALSE))
			return
