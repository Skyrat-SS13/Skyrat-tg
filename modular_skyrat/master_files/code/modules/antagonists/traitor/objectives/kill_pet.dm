...
/datum/traitor_objective/kill_pet/generate_objective(datum/mind/generating_for, list/possible_duplicates)
	...
	if(istype(target_pet, /mob/living/simple_animal/pet/poppy))
		...
		for(var/datum/component/traitor_objective_register/objective as anything in GetComponents(/datum/component/traitor_objective_register))
			if(objective.target == target_pet)
				qdel(objective)
