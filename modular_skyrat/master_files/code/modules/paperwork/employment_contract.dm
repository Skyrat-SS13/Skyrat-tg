...
/obj/item/paper/work_contract/Initialize(mapload, new_employee_name)
	...
	add_raw_text("\
	<center>Conditions of Employment</center>\
	<BR><BR><BR><BR>\
	...
	<BR>WITNESSETH:\
	<BR>WHEREAS, the <i>employee</i> is a natural born human, acceptable humanoid, or synthetic humanoid with proven sapience, possessing skills upon which the <i>employee</i> \
	can aid the <i>employer</i>, and seeks employment with them.\
	<BR>WHEREAS, <i>employer</i> agrees to provide payment to the <i>employee</i> to the best of its ability depending on local circumstance and in a timely fashion, \
	in exchange for the completion of work duties described by the individual job postings to which the <i>employee</i> is assigned.\
	...
	<BR>In exchange for a fixed wage, the <i>employee</i> agrees to work for the <i>employer</i>, for a standard contract of no less than four years.\
	<BR>The employee agrees to maintain confidentiality of all <i>employer</i> trade secrets, locations of stations or other strategically valuable items, \
	value or condition of assets, existence of pending lawsuits, or any other material that may impact the profit and health of the <i>employer</i> as a whole.\
	<BR>Additionally, the <i>employee</i> allows the <i>employer</i> to access their medical, security and employment records for use in public relations, marketing, \
	...
	<BR>In case of lack of necessary skills to perform required tasks before the employment, the <i>employee</i> agrees to perform a skillchip installment and/or \
	...
	medical fees, as well as the skillchip and/or memory altering procedures cost will be deducted from the <i>employee</i>'s payment at the end of the contract.\
	<BR>To note for the aforementioned, any ongoing or pending criminal investigations, if originated from the Sol Federation, are to be halted for the duration of work contract, \
	with any possible crimes occuring within the <i>employer</i>'s facilities punished by the contracted or corporate paramilitary and security forces.\
	<BR>Finally, the <i>employee</i> understands that the <i>employer</i> is not bound by the same contract principles as them and may terminate the contract at any time, \
