	...
	for(var/category in GLOB.position_categories)
		...
		for(var/job in GLOB.position_categories[category]["jobs"])
			...
			if(job_datum && IsJobUnavailable(job_datum.title, TRUE) == JOB_AVAILABLE)
				...
				var/jobline = "[job_datum.title] ([job_datum.current_positions])" // alt title working comment
				if(job_datum in SSjob.prioritized_jobs)
					jobline = "<span class='priority'>[jobline]</span>" // alt title working comment
				...
				/* Alt title working comment - removed bits
