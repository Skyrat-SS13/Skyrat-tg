	...
	switch(rand(1,10))
		if(1 to 3)
			if(!HAS_TRAIT(H, TRAIT_NOHUNGER))
				H.adjust_nutrition(-delta_time*10*strength)
		if(3 to 5)
			affected_carbon.emote("cough")
		if(5 to 7)
			to_chat(affected_mob, "<span class='warning'>Your head hurts.</span>")
			affected_mob.adjustStaminaLoss(4*strength)
		if(8)
			if(strength>=2)
				to_chat(affected_mob, "<span class='warning'>You feel a little dizzy.</span>")
				affected_mob.Dizzy(3*strength)
		if(8 to 10)
			to_chat(affected_mob, "<span class='warning'>You feel tired.</span>")
			affected_mob.adjustStaminaLoss(6*strength)
