...
/datum/datacore/manifest_inject(mob/living/carbon/human/human, client/human_client)
	...
	// Purposefully catches normal off-manifest roles and sends them to the ghost role manifest to allow for easier passport generation.
	if(!(/datum/background_feature/off_manifest in initial(background_info.features)) && human.mind?.assigned_role.job_flags & JOB_CREW_MANIFEST)
		return ..()
