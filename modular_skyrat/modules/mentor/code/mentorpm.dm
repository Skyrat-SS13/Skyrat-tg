...
/client/proc/cmd_mentor_pm(whom, msg)
	...
	if(is_mentor(whom))
		to_chat(GLOB.mentors, span_mentor("<font color='purple'>[src] has started replying to [whom]'s mhelp.</font>"))
	...
	if(C.is_mentor())
		if(is_mentor())//both are mentors
			to_chat(C, span_mentor(span_purple("Mentor PM from-<b>[key_name_mentor(src, C, 1, 0, 0)]</b>: [msg]")))
			to_chat(src, span_mentor(span_blue("Mentor PM to-<b>[key_name_mentor(C, C, 1, 0, 0)]</b>: [msg]")))

		else		//recipient is a mentor but sender is not
			to_chat(C, span_mentor(span_purple("Reply PM from-<b>[key_name_mentor(src, C, 1, 0, show_char)]</b>: [msg]")))
			to_chat(src, span_mentor(span_pink("Mentor PM to-<b>[key_name_mentor(C, C, 1, 0, 0)]</b>: [msg]")))

	else
		if(is_mentor())	//sender is a mentor but recipient is not.
			to_chat(C, span_mentor(span_purple("Mentor PM from-<b>[key_name_mentor(src, C, 1, 0, 0)]</b>: [msg]")))
			to_chat(src, span_mentor(span_pink("Mentor PM to-<b>[key_name_mentor(C, C, 1, 0, show_char)]</b>: [msg]")))
	...
	for(var/it in GLOB.mentors)
		...
		if(X.key!=key && X.key!=C.key)	//check client/X is an Mentor and isn't the sender or recipient
			to_chat(X, span_mentor("<B><span class='pink'>Mentor PM: [key_name_mentor(src, X, 0, 0, show_char_sender)]-&gt;[key_name_mentor(C, X, 0, 0, show_char_recip)]:</span></B> <span class='blue'>[msg]</span>")) //inform X
