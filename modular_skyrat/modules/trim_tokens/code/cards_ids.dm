...
/obj/item/card/id/proc/apply_token(obj/item/trim_token/token, mob/user)
	if(token.has_required_trim)
		if(!(src.trim in token.valid_trims))
			to_chat(user, "<span class='warning'>The trim of your ID is not valid for this trim token.</span>")
			return
	if(token.uses == 0)
		to_chat(user, "<span class='warning'>The [token.name] disintegrates as you press it against your ID, it probably only barely had enough charge left to keep its shape!</span>")
		qdel(token)
		return
	...
	if(SSid_access.apply_trim_to_card(src, token.token_trim, copy_access = token.force_access))
		...
		if(token.uses == 0)
			qdel(token)
			return
		to_chat(user, "<span class='notice'>The [token.name] reforms into a solid token before your eyes, after having successfully replaced your ID's trim. Nice.</span>")
		return
	else
