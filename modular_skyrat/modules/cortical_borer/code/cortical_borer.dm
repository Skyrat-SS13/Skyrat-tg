	...
	if(cb_inside.body_focus)
		switch(cb_inside.body_focus)
			if(FOCUS_HEAD)
				...
				carbon_target.add_quirk(/datum/quirk/night_vision)
				carbon_target.dna.add_mutation(XRAY)
			if(FOCUS_CHEST)
...
/obj/item/organ/borer_body/Remove(mob/living/carbon/M, special)
	. = ..()
	var/mob/living/simple_animal/cortical_borer/cb_inside = M.has_borer()
	if(cb_inside?.body_focus)
		switch(cb_inside.body_focus)
			if(FOCUS_HEAD)
				to_chat(M, span_notice("Your eyes begin to return to normal..."))
				M.remove_quirk(/datum/quirk/night_vision)
				M.dna.remove_mutation(XRAY)
			if(FOCUS_CHEST)
				to_chat(M, span_notice("Your chest begins to heave again..."))
				REMOVE_TRAIT(M, TRAIT_NOBREATH, src)
				REMOVE_TRAIT(M, TRAIT_NOHUNGER, src)
				REMOVE_TRAIT(M, TRAIT_STABLEHEART, src)
				REMOVE_TRAIT(M, TRAIT_HARDLY_WOUNDED, src)
			if(FOCUS_ARMS)
				to_chat(M, span_notice("Your arm starts to feel normal again..."))
			...
			if(FOCUS_LEGS)
				to_chat(M, span_notice("You feel slower..."))
				M.remove_movespeed_modifier(/datum/movespeed_modifier/borer_speed)
				M.remove_quirk(/datum/quirk/light_step)
	if(cb_inside)
		cb_inside.leave_host()
	qdel(src)
