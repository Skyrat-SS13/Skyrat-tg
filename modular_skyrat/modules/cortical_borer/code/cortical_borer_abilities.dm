...
/datum/action/cooldown/inject_chemical/Trigger()
	...
	cortical_owner.log_message(logging_text)
	cortical_owner.human_host.log_message(logging_text)
	StartCooldown()
...
/datum/action/cooldown/fear_human/Trigger()
	...
	if(length(potential_freezers) == 1)
		...
		cortical_owner.log_message(logging_text)
		singular_fear.log_message(logging_text)
		StartCooldown()
		return
	...
	cortical_owner.log_message(logging_text)
	choose_fear.log_message(logging_text)
	StartCooldown()
...
/datum/action/cooldown/produce_offspring/Trigger()
	...
	cortical_owner.log_message(logging_text)
	spawn_borer.log_message(logging_text)
	StartCooldown()
