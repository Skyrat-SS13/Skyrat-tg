...
/datum/action/cooldown/borer/inject_chemical/ui_act(action, params)
	...
	switch(action)
		...
		if("inject")
			...
			cortical_owner.human_host.hurt_organ_randomly(rand(1,10))
...
/datum/action/cooldown/borer/produce_offspring/Trigger(trigger_flags)
	...
	cortical_owner.human_host.hurt_organ_randomly(rand(1, 30)) // Laying eggs gotta hurt!
	to_chat(cortical_owner.human_host, span_warning("Your body begins to hurt..."))
...
/datum/action/cooldown/borer/learn_ability/Trigger(trigger_flags)
	...
	for(var/ability in abil_list)
		switch(ability)
			...
			if("Willing Host")
				if(locate(/datum/action/cooldown/borer/gift_host) in cortical_owner.known_abilities)
	...
	switch(ability_choice)
		...
		if("Revive Host")
			...
			return
		if("Willing Host")
			var/datum/action/attack_action = new /datum/action/cooldown/borer/gift_host()
			attack_action.Grant(cortical_owner)
			cortical_owner.known_abilities += /datum/action/cooldown/borer/gift_host
			return
		if("Upgrade Injection")
...
/datum/action/cooldown/borer/gift_host
	name = "Gift Host (200 chemicals)"
	cooldown_time = 5 MINUTES
	button_icon_state = "willing"
	var/required_chems = 200

/datum/action/cooldown/borer/gift_host/Trigger(trigger_flags)
	. = ..()
	if(!.)
		return FALSE
	var/mob/living/simple_animal/cortical_borer/cortical_owner = owner
	...
	if(cortical_owner.host_sugar())
		owner.balloon_alert(owner, "cannot function with sugar in host")
		return
	if(cortical_owner.chemical_storage < required_chems)
		owner.balloon_alert(owner, "[required_chems] chemical points required")
		return
	if(cortical_owner.human_host.mind.has_antag_datum(/datum/antagonist/changeling))
		to_chat(owner, "Host is already a changeling!")
		return
	cortical_owner.chemical_storage -= required_chems
	if(do_after(cortical_owner, 15 SECONDS) && !cortical_owner.human_host.mind.has_antag_datum(/datum/antagonist/changeling))
		GLOB.gifted_hosts += cortical_owner.human_host.ckey
		cortical_owner.human_host.mind.add_antag_datum(/datum/antagonist/changeling)
	owner.balloon_alert(owner, "host willing!")
	to_chat(cortical_owner.human_host, span_spider("You feel elastic..."))
	StartCooldown()
