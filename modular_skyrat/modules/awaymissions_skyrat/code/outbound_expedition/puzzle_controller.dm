/datum/outbound_puzzle_controller
	/// Assoc list of initialized puzzle datums ("name" : reference)
	...
	var/list/possible_conditionals = list(
		/datum/outbound_wire_conditional/simple_act = 2,
...
/datum/outbound_puzzle_controller/New()
	...
	addtimer(CALLBACK(src, PROC_REF(set_up_wire_conditionals)), 1 SECONDS)

	RegisterSignal(src, COMSIG_AWAY_PUZZLE_COMPLETED, PROC_REF(puzzle_finish))
...
/*
/datum/outbound_puzzle_controller/proc/set_up_wire_conditionals_()
...
/datum/outbound_puzzle_controller/proc/set_up_wire_conditionals()
	var/conditional_amount = rand(2, 3)
	var/compiled_desc = ""
	var/datum/outbound_teamwork_puzzle/wires/wire_puzzle = puzzles["Wires"]
	for(var/i in 1 to conditional_amount)
		var/conditional = pick_weight(possible_conditionals)
	...
	wire_puzzle.desc = compiled_desc
