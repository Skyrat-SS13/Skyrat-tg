//KubeRoot is a byond magician and his name deserves to be immortalized for this masterpiece.
/client/proc/create_input_window(id, title, accept_verb, cancel_verb, force=FALSE, show=TRUE)
	...
	if(accept_verb)
		...
		winset(src, "[id].accept", "command=\".winset \\\"command=\\\"[accept_verb] \\\\\\\"\[\[[id].input.text as escaped\]\]\\\";[id].is-visible=false\\\"\"") //Invokes the accept verb using the inputted text, and hides the window.
	if(cancel_verb)
