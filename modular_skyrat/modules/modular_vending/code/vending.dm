...
/obj/machinery/vending/Initialize(mapload)
	...
	for (var/obj/item/clothing/item in products)
		if(products[item] < MINIMUM_CLOTHING_STOCK && is_item_increase_banned(item))
			products[item] = MINIMUM_CLOTHING_STOCK
	for (var/obj/item/clothing/item in premium)
		if(premium[item] < MINIMUM_CLOTHING_STOCK && is_item_increase_banned(item))
...
/obj/machinery/vending/proc/is_item_increase_banned(obj/item/clothing/clothing_path)
