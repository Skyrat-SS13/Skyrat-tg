...
/datum/nifsoft/summoner/activate()
	. = ..()
	//Clears out any items that don't exist inside of the summoned_items list before we continue.
	for(var/obj/item/summoned_item in summoned_items)
		if(!summoned_item.loc)
			summoned_items -= summoned_item
...
///Adds an examine text to a summoned item, showing that it can be destroyed.
/datum/component/summoned_item/proc/add_examine(mob/nif_user, mob/looker, list/examine_texts)
	SIGNAL_HANDLER

	examine_texts += span_blue("<b>Ctrl + Shift + Click</b> to destroy this item.")
