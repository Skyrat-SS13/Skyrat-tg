...
/datum/extension/sense/New(var/mob/holder, var/range_sense, var/range_buff, var/buff_faction, var/duration, var/cooldown)
	...
	var/obj/effect/effect/expanding_circle/EC = new /obj/effect/effect/expanding_circle(user.loc, 2, 2 SECONDS)
	EC.pixel_y += 40	//Offset it so it appears to be at our mob's head
	spawn(4)
		EC = new /obj/effect/effect/expanding_circle(user.loc, 2, 2 SECONDS)
		EC.pixel_y += 40	//Offset it so it appears to be at our mob's head
		spawn(4)
			EC = new /obj/effect/effect/expanding_circle(user.loc, 2, 2 SECONDS)
			EC.pixel_y += 40	//Offset it so it appears to be at our mob's head
