...
/obj/structure/large_mortar/attackby(obj/item/attacking_item, mob/living/carbon/human/user)
	...
	if(istype(attacking_item, /obj/item/pestle))
		...
		if(!LAZYLEN(contained_items) || !in_range(src, user) || !user.is_holding(attacking_item) && !picked_option)
			return
		...
		switch(picked_option)
			if("Juice")
				for(var/obj/item/target_item in contained_items)
			...
			if("Grind")
				for(var/obj/item/target_item in contained_items)
					if(target_item.grind_results)
						...
						juice_target_item(target_item, user)
		return
	if(LAZYLEN(contained_items) >= maximum_contained_items)
	...
	LAZYADD(contained_items, attacking_item)
