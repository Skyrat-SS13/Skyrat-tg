...
/obj/structure/destructible/clockwork/gear_base/interdiction_lens/process(delta_time)
	...
	for(var/mob/living/L in viewers(INTERDICTION_LENS_RANGE, src))
		if(!(FACTION_CLOCK in L.faction) && use_power(5))
...
/datum/proximity_monitor/advanced/projectile_dampener/peaceborg/clockwork/capture_projectile(obj/projectile/P, track_projectile = TRUE)
	if(P in tracked)
		return
	if(isliving(P.firer))
		var/mob/mob_firer = P.firer
		if(FACTION_CLOCK in mob_firer.faction)
			return
	var/obj/structure/destructible/clockwork/gear_base/interdiction_lens/host_lens = host
	host_lens.internal_dampener.dampen_projectile(P, track_projectile)
	if(track_projectile)
		tracked += P
