...
/datum/preferences
	...
	// Alternate Job Titles - oh god save me
	...
	if(SSjob.occupations.len <= 0)
		...
		for(var/datum/job/job in sortList(SSjob.occupations, /proc/cmp_job_display_asc))
			...
			if((job_preferences[SSjob.overflow_role] == JP_LOW) && (rank != SSjob.overflow_role) && !is_banned_from(user.ckey, SSjob.overflow_role))
				...
				continue
			//alt titles working comment
			var/rank_title_line = "[displayed_rank]"
			if((rank in GLOB.command_positions) || (rank == "AI"))//Bold head jobs - alt titles working comment original line, not part of change
			...
			else //alt titles working comment, original line
			...
			// alt titles working comment end
