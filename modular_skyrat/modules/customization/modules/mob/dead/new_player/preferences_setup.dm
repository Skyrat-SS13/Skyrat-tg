...
/datum/preferences/proc/equip_preference_loadout(mob/living/carbon/human/H, just_preview = FALSE, datum/job/choosen_job, blacklist)
	if(!ishuman(H))
		return
	...
	for(var/item_name in loadout)
		...
		if(!just_preview && (choosen_job && LI.restricted_roles && !(choosen_job.title in LI.restricted_roles)))
			continue
		if(!H.equip_to_appropriate_slot(ITEM,blacklist=blacklist))
			if(!just_preview)
				items_to_pack += ITEM
