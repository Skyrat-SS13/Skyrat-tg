...
/datum/sprite_accessory
	 ...
	 * Also can be an assoc list of layers to a list of valid extras that indicate to the sprite accessory system what layers and extras combos are in use.
	 * Valid extras are: `SPRITE_ACCESSORY_PRIMARY`, `SPRITE_ACCESSORY_EXTRA2`, `SPRITE_ACCESSORY_INNER`, and if you're using matrixed, just use the name of your color layer.
	 * This is normally autogenerated, so only make this associative if you're doing something like cat ears, where certain extras (inner) aren't used in certain layers.
	 * Valid example: `list(BODY_FRONT_LAYER, BODY_ADJ_LAYER)`
	 * OR, if you don't want a certain sprite extra to render on certain layers (e.g, you don't want inner to render on the behind layer): `list(BODY_FRONT_LAYER, BODY_BEHIND_LAYER = list(SPRITE_ACCESSORY_PRIMARY))`
	 */
