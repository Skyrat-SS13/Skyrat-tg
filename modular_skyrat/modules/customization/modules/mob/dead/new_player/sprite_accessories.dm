...
/datum/sprite_accessory
	 ...
	 * Valid list entries are `BODY_BEHIND_LAYER`, `BODY_ADJ_LAYER`, `BODY_FRONT_LAYER`, `BODY_FRONT_UNDER_CLOTHES` and `ABOVE_BODY_FRONT_HEAD_LAYER`
	 * Valid example: `list(BODY_FRONT_LAYER, BODY_ADJ_LAYER)`
	 */
	var/list/relevent_layers

	/**
	 * An optional assoc list of layers to a list of valid extras that indicate to the sprite accessory system what layers and extras combos are in use.
	 * Valid extras are: `SPRITE_ACCESSORY_MAIN`, `SPRITE_ACCESSORY_EXTRA`, `SPRITE_ACCESSORY_EXTRA2`, `SPRITE_ACCESSORY_INNER`, and if you're using matrixed, the name of your color layer.
	 * This adds marginally extra overhead if filled out, so only fill this out if you're doing something like cat ears, where certain extras (inner) aren't used in certain layers.
	 * If a layer is not in this list, it will be treated as if there is no filter for that layer.
