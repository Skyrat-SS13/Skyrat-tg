...
/obj/structure/biohazard_blob/structure/spawner/Initialize(mapload)
	...
	switch(blob_type)
		if(BIO_BLOB_TYPE_FUNGUS)
			monster_types = list(/mob/living/basic/mold/diseased_rat)
			spawn_cooldown = 500
		if(BIO_BLOB_TYPE_FIRE)
			monster_types = list(/mob/living/basic/mold/oil_shambler)
		if(BIO_BLOB_TYPE_EMP)
			monster_types = list(/mob/living/basic/mold/electric_mosquito)
			spawn_cooldown = 500
		if(BIO_BLOB_TYPE_TOXIC)
			monster_types = list(/mob/living/basic/giant_spider)
		if(BIO_BLOB_TYPE_RADIOACTIVE)
			monster_types = list(/mob/living/basic/mold/centaur)
