#define BORG_LICK_EXTRA_RANGE -1
...
/obj/item/dogborg_tongue/afterattack(atom/target, mob/user, proximity)
	...
	if(iscarbon(target)) // Is the target carbon?
		...
		if(HAS_TRAIT(carbon,TRAIT_PERSONALSPACE)) // Does the target have the Personal Space trait?
			...
			if(carbon.combat_mode && (!HAS_TRAIT(carbon, TRAIT_PACIFISM)) && (carbon.stat != UNCONSCIOUS) && (!carbon.handcuffed)) // Combat mode slap - must be conscious, unpacified and uncuffed
				...
				playsound(carbon.loc, 'sound/effects/snap.ogg', 50, TRUE, BORG_LICK_EXTRA_RANGE)
		...
		else // If the carbon does NOT have the Personal Space trait, proceed as normal
