...
/datum/reagent/drug/heroin/on_mob_life(mob/living/carbon/M, seconds_per_tick, times_fired)
	...
	if(SPT_PROB(2.5, seconds_per_tick))
		to_chat(M, span_notice("[high_message]"))
	M.add_mood_event("smacked out", /datum/mood_event/narcotic_heavy, name)
	M.adjustBruteLoss(-0.5 * REM * seconds_per_tick, 0) //more powerful as a painkiller, possibly actually useful to medical now
...
/datum/reagent/drug/blacktar/on_mob_life(mob/living/carbon/M, seconds_per_tick, times_fired)
	...
	M.adjustFireLoss(-0.1 * REM * seconds_per_tick, 0)
	M.adjustToxLoss(0.75 * REM * seconds_per_tick, 0) //toxin damage
	return ..()
