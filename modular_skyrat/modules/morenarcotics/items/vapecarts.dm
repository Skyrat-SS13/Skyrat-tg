...
/obj/item/reagent_containers/vapecart/afterattack(atom/target, mob/user, proximity)
	...
	if(istype(target, /obj/item/clothing/mask/vape))
		...
		if(target_vape.screw == TRUE && !target_vape.reagents.total_volume)
			...
			qdel(src)
			to_chat(user, "<span class='notice'>You plug the [src.name] into the vape.</span>")
		else if(!target_vape.screw)
			to_chat(user, "<span class='warning'>You need to open the cap to do that!</span>")
		else
			to_chat(user, "<span class='warning'>[target_vape] is already full!</span>")
...
/obj/item/reagent_containers/vapecart/empty/attackby(obj/item/I, mob/user, params)
	if (istype(I, /obj/item/pen) || istype(I, /obj/item/toy/crayon))
		if(!user.is_literate())
			to_chat(user, "<span class='notice'>You scribble illegibly on the label of the vape cart!</span>")
			return
		var/t = stripped_input(user, "What would you like to label the vape cart?", name, null, 53)
...
/obj/item/reagent_containers/vapecart/bluekush
	...
	list_reagents = list(/datum/reagent/drug/thc = 15, /datum/reagent/consumable/berryjuice = 10, /datum/reagent/toxin = 5) //i mean... it isn't realistic if it doesnt slowly kill you
...
/obj/item/reagent_containers/vapecart/reddiesel
	...
	list_reagents = list(/datum/reagent/drug/thc = 15, /datum/reagent/consumable/dr_gibb = 10, /datum/reagent/toxin = 5)