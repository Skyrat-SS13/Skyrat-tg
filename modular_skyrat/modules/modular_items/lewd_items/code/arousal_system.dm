...
/datum/status_effect/body_fluid_regen/tick()
	...
	if(owner.stat != DEAD && H.client?.prefs?.read_preference(/datum/preference/toggle/sex_toy))
...
/mob/living/carbon/human/proc/adjustArousal(arous = 0)
	if(stat != DEAD && client?.prefs?.read_preference(/datum/preference/toggle/sex_toy))
		arousal += arous
...
/mob/living/carbon/human/proc/climax(manual = TRUE)
	...
	if(manual == TRUE && !has_status_effect(/datum/status_effect/climax_cooldown) && client?.prefs?.read_preference(/datum/preference/toggle/sex_toy))
	...
	else if(manual == FALSE && client?.prefs?.read_preference(/datum/preference/toggle/sex_toy))
		if(!HAS_TRAIT(src, TRAIT_NEVERBONER) && !has_status_effect(/datum/status_effect/climax_cooldown))
			switch(gender)
				if(MALE)
					playsound(get_turf(src), pick('modular_skyrat/modules/modular_items/lewd_items/sounds/final_m1.ogg',
...
/datum/status_effect/climax/on_apply(obj/target)
	...
	if((H.client?.prefs?.read_preference(/datum/preference/toggle/sex_toy)) && (H.stat != DEAD))
...
/obj/item/organ/brain/on_life(delta_time, times_fired) //All your horny is here *points to the head*
	...
	if(istype(brain_owner, /mob/living/carbon/human) && brain_owner.client?.prefs?.read_preference(/datum/preference/toggle/sex_toy))
...
/datum/species/proc/handle_arousal(mob/living/carbon/human/target_human, atom/movable/screen/alert/aroused_X)
	...
	if(target_human.client?.prefs?.read_preference(/datum/preference/toggle/sex_toy))
