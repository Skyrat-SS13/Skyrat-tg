		...
		if(held?.name == "dildo side" && held?.item_flags == ABSTRACT | HAND_ITEM)
			...
			if(unheld.name =="dildo side" && unheld.item_flags == ABSTRACT | HAND_ITEM)
				if(src == C.belt)
					...
					W.update_icon_state()
					W.update_icon()
					C.visible_message("<span class='notice'>[user] holds one end of [src] in [user.p_their()] hand.</span>")
					in_hands = TRUE
					return
		else
			C.visible_message("<span class='notice'>[user] tries to hold one end of [src] in [user.p_their()] hand, but [user.p_their()] hand isn't empty!</span>")
			return
	else
		W = new()
		C.put_in_hands(W)
