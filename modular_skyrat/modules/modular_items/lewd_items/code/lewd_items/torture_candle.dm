	...
	switch(user.zone_selected) //to let code know what part of body we gonna wax
		...
		if(BODY_ZONE_CHEST)
			...
			if(attacked.is_topless() || breasts.visibility_preference == GENITAL_ALWAYS_SHOW)
				message = (user == attacked) ? pick("drips some wax on [attacked.p_their()] [breasts ? ORGAN_SLOT_BREASTS : ORGAN_SLOT_NIPPLES], releasing all [attacked.p_their()] lustness", "drips some wax right on [attacked.p_their()] [breasts ? "tits" : "chest"], making [attacked.p_their()] feel faint.") : pick("pours the wax that is slowly dripping from [src] onto [attacked]'s [breasts ? ORGAN_SLOT_BREASTS : ORGAN_SLOT_NIPPLES], [attacked.p_they()] shows pure enjoyment.", "tilts the candle. Right in the moment when wax drips on [attacked]'s [breasts ? ORGAN_SLOT_BREASTS : ORGAN_SLOT_NIPPLES], [attacked.p_they()] shivers", "tilts the candle. Just when hot drops of wax fell on [attacked]'s [breasts ? ORGAN_SLOT_BREASTS : ORGAN_SLOT_NIPPLES], [attacked.p_they()] quietly moans in pleasure")
