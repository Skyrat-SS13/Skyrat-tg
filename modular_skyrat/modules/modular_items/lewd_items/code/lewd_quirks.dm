	...
	if(satisfaction <= 0)
		if(prob(10))
			switch(rand(1,6))
				...
				if(5)
					if(stress >= 100)
						to_chat(owner, span_purple("You feel like your genitalias are burning..."))
						owner.blur_eyes(10)
						owner.adjustStaminaLoss(20)
					else
						to_chat(owner, span_purple("Your body pulses with lust, you need release!"))
						owner.blur_eyes(15)
...
/datum/brain_trauma/special/nymphomania/proc/in_company()
	...
	for(var/mob/living/carbon/human/M in oview(owner, 4))
		if((M.stat == CONSCIOUS) || !(M.client) || ismonkey(M)) //corpses, empty, consciousness-less husks/ssd players and FOR THE LOVE OF FUCKING CHRIST, MONKEYS, ain't people
			continue
		if(istype(M))
			return TRUE
	return FALSE
