...
/datum/quirk/hypnotic
	...
	gain_text = span_notice("You find yourself able to captivate others into trance.")
	lose_text = span_notice("You lose the ability to mesmorize others.")
	erp_quirk = TRUE
...
/datum/action/innate/hypnotize/Activate()
	...
	var/mob/living/carbon/target = source.pulling
	if(!target.client?.prefs?.read_preference(/datum/preference/toggle/erp/sex_toy))
		to_chat(source, span_danger("[target] doesn't want to be hypnotized."))
		return

	if(target.IsSleeping())
		to_chat(source, span_warning("You can't hypnotize [target] while they're asleep."))
		return

	if(!do_mob(source, target, 12 SECONDS))
		return

	source.visible_message(span_warning("[source] begins to gradually pull [target] down into a hypnotic trance!"))
	var/response = tgui_alert(source, "Do you belive in hypnosis? (This will allow [source] to issue hypnotic suggestions.)", "Hypnosis", list("Yes", "No"))
	if(response == "Yes")
		target.visible_message(span_purple("[target] falls into a deep, hypnotic slumber right at the snap of your finger."), span_purple("You suddenly fall limp at the snap of [source]'s fingers."))
		target.SetSleeping(60 SECONDS)
		var/response2 = tgui_alert(target, "Would you like to give [target] a hypnotic suggestion or release them?", "Hypnosis", list("Suggestion", "Release"))
		if(response2 == "Suggestion")
			if(in_range(source, target))
				to_chat(source, span_warning("You must be in whisper range to [target] in order to give hypnotic suggestions."))
				return
			var/text = tgui_input_text(source, "What would you like to suggest?", "Hypnotic Suggestion")
			to_chat(source, span_purple("You whisper into [target] ears in a soothing voice."))
			to_chat(target, span_hypnophrase("[text]"))
			target.SetSleeping(5 SECONDS) // Once you give a suggestion, wake them up
		else
			target.visible_message(span_purple("[target] wakes up from their slumber."), span_purple("You wake up from your deep, hypnotc slumber and the new compulsions from [source] settle into your mind."))
			target.SetSleeping(0)
			target = null
	else
		target.visible_message(span_warning("[target]'s attention breaks despite your efforts. They clearly don't seem interested!"), span_danger("Your attention breaks as you realize that you don't want to listen to [source]'s suggestions."))
