/proc/try_warp_servant(mob/living/servant, turf/target_location, bring_dragging = FALSE)
	...
	do_teleport(M, target_location, channel = TELEPORT_CHANNEL_FREE, no_effects = TRUE)
	...
	if(istype(P) && bring_dragging)
		do_teleport(P, target_location, channel = TELEPORT_CHANNEL_FREE, no_effects = TRUE)
		P.Paralyze(30)
