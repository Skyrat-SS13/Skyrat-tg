	...
	switch(level)
		...
		if(SEC_LEVEL_VIOLET)
			if(SSsecurity_level.current_level < SEC_LEVEL_VIOLET)
				minor_announce(CONFIG_GET(string/alert_violet_upto), "Attention! Alert level set to violet:", TRUE, sound = 'modular_skyrat/modules/alerts/sound/misc/voyalert.ogg', TRUE)
			else
				minor_announce(CONFIG_GET(string/alert_violet_downto), "Attention! Alert level set to violet:", sound = 'modular_skyrat/modules/alerts/sound/misc/voyalert.ogg', TRUE)
		if(SEC_LEVEL_ORANGE)
			if(SSsecurity_level.current_level < SEC_LEVEL_ORANGE)
				minor_announce(CONFIG_GET(string/alert_orange_upto), "Attention! Alert level set to orange:", TRUE, sound = 'modular_skyrat/modules/alerts/sound/misc/voyalert.ogg', TRUE)
			else
				minor_announce(CONFIG_GET(string/alert_orange_downto), "Attention! Alert level set to orange:", sound = 'modular_skyrat/modules/alerts/sound/misc/voyalert.ogg', TRUE)
		if(SEC_LEVEL_AMBER)
			if(SSsecurity_level.current_level < SEC_LEVEL_AMBER)
				minor_announce(CONFIG_GET(string/alert_amber_upto), "Attention! Alert level set to amber:", TRUE, sound = 'modular_skyrat/modules/alerts/sound/misc/voyalert.ogg', TRUE)
			else
				minor_announce(CONFIG_GET(string/alert_amber_downto), "Attention! Alert level set to amber:", sound = 'modular_skyrat/modules/alerts/sound/misc/voyalert.ogg', TRUE)
		if(SEC_LEVEL_RED)
		...
		if(SEC_LEVEL_DELTA)
			if(SSsecurity_level.current_level < SEC_LEVEL_DELTA)
				minor_announce(CONFIG_GET(string/alert_delta_upto), "Attention! Delta Alert level reached!", TRUE, sound = 'modular_skyrat/modules/alerts/sound/misc/delta.ogg', TRUE)
			else
				minor_announce(CONFIG_GET(string/alert_delta_downto), "Attention! Delta Alert level reached!", TRUE, sound = 'modular_skyrat/modules/alerts/sound/misc/delta.ogg', TRUE)
	...
	for(var/m in GLOB.player_list)
		if(ismob(m) && !isnewplayer(m))
			...
			if(M.client.prefs.toggles & SOUND_ANNOUNCEMENTS && M.can_hear())
				if(override_volume)
					M.playsound_local(get_turf(M), S, 100, FALSE)
				else
