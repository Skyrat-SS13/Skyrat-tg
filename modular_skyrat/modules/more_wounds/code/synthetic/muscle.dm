...
*/
/datum/wound/synthetic/muscle/wound_injury(datum/wound/old_wound = null)
	...
	if(limb.held_index && victim.get_item_for_held_index(limb.held_index) && (disabling || prob(30 * severity)))
		...
		if(I && victim.dropItemToGround(I))
			victim.visible_message("<span class='danger'>[victim] drops [I] as their limb malfunctions!</span>", "<span class='warning'><b>The force on your [limb.name] causes you to drop [I]!</b></span>", vision_distance=COMBAT_MESSAGE_RANGE)

	update_inefficiencies()

/datum/wound/synthetic/muscle/remove_wound(ignore_limb, replaced)
	limp_slowdown = 0
	if(limb)
...
/datum/wound/synthetic/muscle/handle_process()
	. = ..()
	...
	if(regen_ticks_current > regen_ticks_needed)
		if(!victim || !limb)
			qdel(src)
			return
		to_chat(victim, "<span class='green'>Your [limb.name] has repaired its hydraulics!</span>")
		remove_wound()

/// If we're a human who's punching something with a broken arm, we might hurt ourselves doing so
/datum/wound/synthetic/muscle/proc/attack_with_hurt_hand(mob/M, atom/target, proximity)
	SIGNAL_HANDLER
	...
	if(prob(severity * 15))
		...
		if(prob(70 - 20 * severity))
			to_chat(victim, "<span class='userdanger'>The control circuits in your [limb.name] malfunction as you strike [target], electricity arcing violently!</span>")
			limb.receive_damage(brute=rand(1,5))
		else
			victim.visible_message("<span class='danger'>[victim] weakly strikes [target] with [victim.p_their()] malfunctioning [limb.name], recoiling as it smokes and pops weakly!</span>", \
			"<span class='userdanger'>You fail to strike [target] as the fractured hydraulics in your [limb.name] light up with multiple warnings: your internals failing!</span>", vision_distance=COMBAT_MESSAGE_RANGE)
			INVOKE_ASYNC(victim, /mob.proc/emote, "scream")
			victim.Stun(0.5 SECONDS)
			limb.receive_damage(brute=rand(3,7))
			return COMPONENT_CANCEL_ATTACK_CHAIN

/datum/wound/synthetic/muscle/get_examine_description(mob/user)
	if(!limb.current_splint)
		return ..()
...
/datum/wound/synthetic/muscle/proc/update_inefficiencies()
	SIGNAL_HANDLER
...
/datum/wound/synthetic/muscle/moderate
	name = "Hydraulic Damage"
	desc = "Patient's hydraulics have been damaged, severely reducing limb functionality"
	treat_text = "Splinting of the limb, allowing it to properly self-repair"
	examine_desc = "appears to have been damaged, its hydraulics leaking fluid"
	occur_text = "sparks, beginning to leak oil"
	severity = WOUND_SEVERITY_MODERATE
	interaction_efficiency_penalty = 1.5
	limp_slowdown = 2
	threshold_minimum = 35
	threshold_penalty = 15
	status_effect_type = /datum/status_effect/wound/synthetic/muscle/moderate
...
/datum/wound/synthetic/muscle/severe
	name = "Shattered Hydraulics"
	sound_effect = 'sound/effects/wounds/blood2.ogg'
	desc = "Patient's Hydraulic Line has been severed, causing loss of pressure, and loss of use of the limb."
	treat_text = "Splint and non-use of the limb"
	examine_desc = "is limp, weakly sparking and leaking oil"
	occur_text = "twists; violently screeching as several internal components twist in the wrong ways"
	...
	disabling = TRUE
	status_effect_type = /datum/status_effect/wound/synthetic/muscle/severe
	regen_ticks_needed = 150

/datum/status_effect/wound/synthetic/muscle

/datum/status_effect/wound/synthetic/muscle/on_apply()
	. = ..()
	RegisterSignal(owner, COMSIG_MOB_SWAP_HANDS, .proc/on_swap_hands)
	on_swap_hands()

/datum/status_effect/wound/synthetic/muscle/on_remove()
	. = ..()
...
/datum/status_effect/wound/synthetic/muscle/proc/on_swap_hands()
	SIGNAL_HANDLER
...
/datum/status_effect/wound/synthetic/muscle/nextmove_modifier()
...
// muscle
/datum/status_effect/wound/synthetic/muscle/moderate
	id = "torn muscle"
