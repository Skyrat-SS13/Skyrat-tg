...
/datum/brain_trauma/special/obsessed/on_life(seconds_per_tick, times_fired)
	...
	if(obsession in view(7, owner))
		...
		viewing = FALSE
	if(viewing)
		owner.add_mood_event("creeping", /datum/mood_event/creeping)
...
/datum/brain_trauma/special/obsessed/proc/out_of_view()
	...
	if(time_spent_away > 1800) //3 minutes
		owner.add_mood_event("creeping", /datum/mood_event/notcreepingsevere)
	else
		owner.add_mood_event("creeping", /datum/mood_event/notcreeping)
