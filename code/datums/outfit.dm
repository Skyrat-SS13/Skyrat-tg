...
/datum/outfit/proc/load_from(list/outfit_data)
	...
	log_cloning("Loading outfit named \"[name]\":")
	uniform = text2path(outfit_data["uniform"])
	log_cloning("uniform: [uniform]")
	suit = text2path(outfit_data["suit"])
	log_cloning("suit: [suit]")
	toggle_helmet = outfit_data["toggle_helmet"]
	log_cloning("toggle_helmet: [toggle_helmet]")
	back = text2path(outfit_data["back"])
	log_cloning("back: [back]")
	belt = text2path(outfit_data["belt"])
	log_cloning("belt: [belt]")
	gloves = text2path(outfit_data["gloves"])
	log_cloning("gloves: [gloves]")
	shoes = text2path(outfit_data["shoes"])
	log_cloning("shoes: [shoes]")
	head = text2path(outfit_data["head"])
	log_cloning("head: [head]")
	mask = text2path(outfit_data["mask"])
	log_cloning("mask: [mask]")
	neck = text2path(outfit_data["neck"])
	log_cloning("neck: [neck]")
	ears = text2path(outfit_data["ears"])
	log_cloning("ears: [ears]")
	glasses = text2path(outfit_data["glasses"])
	log_cloning("glasses: [glasses]")
	id = text2path(outfit_data["id"])
	log_cloning("id: [id]")
	id_trim = text2path(outfit_data["id_trim"])
	log_cloning("id_trim: [id_trim]")
	l_pocket = text2path(outfit_data["l_pocket"])
	log_cloning("l_pocket: [l_pocket]")
	r_pocket = text2path(outfit_data["r_pocket"])
	log_cloning("r_pocket: [r_pocket]")
	suit_store = text2path(outfit_data["suit_store"])
	log_cloning("suit_store: [suit_store]")
	r_hand = text2path(outfit_data["r_hand"])
	log_cloning("r_hand: [r_hand]")
	l_hand = text2path(outfit_data["l_hand"])
	log_cloning("l_hand: [l_hand]")
	internals_slot = outfit_data["internals_slot"]
	log_cloning("internals_slot: [internals_slot]")
	...
	for(var/I in impl)
		...
		if(imptype)
			implants += imptype
			log_cloning("implant: [I]")
	accessory = text2path(outfit_data["accessory"])
	log_cloning("accessory: [accessory]")
	log_cloning("Successfully loaded outfit \"[name]\"!")
	return TRUE
