...
/datum/element/forced_gravity/Attach(datum/target, gravity = 1, ignore_turf_gravity = FALSE, can_override = FALSE) //SKYRAT EDIT - TEMP CI FIX
	. = ..()
	if(!isatom(target))
	...
	RegisterSignal(target, COMSIG_ATOM_HAS_GRAVITY, PROC_REF(gravity_check), override = can_override) //SKYRAT EDIT - TEMP CI FIX
	if(isturf(target))
		RegisterSignal(target, COMSIG_TURF_HAS_GRAVITY, PROC_REF(turf_gravity_check), override = can_override) //SKYRAT EDIT - TEMP CI FIX
