	...
	for(var/datum/data/record/record as anything in GLOB.data_core.general)
		...
		if(!job || !(job.job_flags & JOB_CREW_MANIFEST) || !LAZYLEN(job.departments_list)) // In case an unlawful custom rank is added.
			...
			misc_list[++misc_list.len] = list(
				"name" = name,
				"rank" = rank,
				"trim" = trim, // SKYRAT CHANGE ADDITION - Alternative Job Titles
				)
			continue
...
/datum/datacore/proc/get_manifest_html(monochrome = FALSE)
	...
	for(var/department in manifest)
		...
		for(var/entry in entries)
			...
			dat += "<tr[even ? " class='alt'" : ""]><td>[entry_list["name"]]</td><td>[entry_list["rank"] == entry_list["trim"] ? entry_list["rank"] : "[entry_list["rank"]] ([entry_list["trim"]])"]</td></tr>" // SKYRAT CHANGE EDIT - Alternative Job Titles - Original: dat += "<tr[even ? " class='alt'" : ""]><td>[entry_list["name"]]</td><td>[entry_list["rank"]]</td></tr>"
			even = !even
...
/datum/datacore/proc/manifest_inject(mob/living/carbon/human/H, client/C)
	...
	if(H.mind?.assigned_role.job_flags & JOB_CREW_MANIFEST)
		...
		G.fields["rank"] = chosen_assignment // SKYRAT EDIT CHANGE - Alternative Job Titles - Original: G.fields["rank"] = assignment
		...
		L.fields["rank"] = chosen_assignment  // SKYRAT EDIT CHANGE - Alternative Job Titles - Original: L.fields["rank"] = assignment
