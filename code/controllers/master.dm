...
/datum/controller/master/proc/Loop(init_stage)
	...
	while (1)
		...
		if(newdrift - olddrift >= CONFIG_GET(number/drift_dump_threshold))
			debug_admins("TICK DRIFT ABOVE [CONFIG_GET(number/drift_dump_threshold)], ATTEMPTING TO DUMP PROFILE")
		...
		if (init_stage == INITSTAGE_MAX)
			...
			if (starting_tick_usage > TICK_LIMIT_MC) //if there isn't enough time to bother doing anything this tick, sleep a bit.
				...
				debug_admins("TICK USAGE ABOVE [TICK_LIMIT_MC], ATTEMPTING TO DUMP PROFILE")
