...
/datum/controller/subsystem/tts/proc/establish_connection_to_tts()
	...
	if(response.errored || response.status_code != 200)
		message_admins("TTS connection failed! Status code [response.status_code]: [response.error]")
	...
	if(response_pitch.errored || response_pitch.status_code != 200)
		if(response_pitch.errored)
			message_admins("TTS server does not support pitch! Status code [response_pitch.status_code]: [response_pitch.error]")
			stack_trace(response.error)
		pitch_enabled = FALSE
