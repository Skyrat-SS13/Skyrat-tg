...
/datum/controller/subsystem/ticker/fire()
	switch(current_state)
		if(GAME_STATE_STARTUP)
			...
			send2chat("<@&[CONFIG_GET(number/gamealertroleid)]> New round starting on [SSmapping.config.map_name], [CONFIG_GET(string/servername)]! \nIf you wish to be pinged for game related stuff, go to a text channel and type $assignrole [CONFIG_GET(string/gamealertrolename)]", CONFIG_GET(string/chat_announce_new_game)) // Skyrat EDIT -- role ping
