...
/datum/controller/subsystem/economy/proc/departmental_payouts()
	...
	for(var/i in 1 to length(cached_processing))
		...
		if(!dept_account)
			continue
		dept_account.adjust_money(MAX_GRANT_DPT * ((wait / 5 MINUTES) / 1 MINUTES)) // SKYRAT EDIT  - BACKGROUNDS - Scaling department grants - Original: dept_account.adjust_money(MAX_GRANT_DPT)
		if(MC_TICK_CHECK)
...
/datum/controller/subsystem/economy/proc/issue_paydays()
	...
	for(var/i in 1 to length(cached_processing))
		...
		if(bank_account?.account_job && !ispath(bank_account.account_job))
			temporary_total += (bank_account.account_job.paycheck * STARTING_PAYCHECKS * ((wait / 5 MINUTES) / 1 MINUTES)) // SKYRAT EDIT  - BACKGROUNDS - Scaling paychecks. - Original: temporary_total += (bank_account.account_job.paycheck * STARTING_PAYCHECKS)
		bank_account.payday(((wait / 5 MINUTES) / 1 MINUTES)) // SKYRAT EDIT  - BACKGROUNDS - Scaling paychecks - ORIGINAL: bank_account.payday(1)
