...
/obj/item/organ/tongue/unique()
	...
	say_mod = lowertext(prefs.speech_verb)
	taste_sensitivity = 10 // combined nose + tongue, extra sensitive
	modifies_speech = TRUE

/obj/item/organ/tongue/unique/handle_speech(datum/source, list/speech_args)
	var/datum/preferences/prefs
	var/replace_upper = uppertext(prefs.speech_replace_from)
	...
	if(message[1] != "*")
		...
		message = replacetext(message, replace_upper,uppertext(prefs.speech_replace_to))
		message = replacetext(message, replace_lower,lowertext(prefs.speech_replace_to))
