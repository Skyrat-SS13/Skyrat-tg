...
/obj/item/organ/tongue/unique(client/prefs/cprefs)
	name = "unique tongue"
...
/obj/item/organ/tongue/unique/handle_speech(datum/source, list/speech_args, client/prefs/cprefs)
	...
	if(message[1] != "*")
		message = custom_quirk_upper.Replace(message, uppertext(cprefs.speech_replace_to))
		message = custom_quirk_lower.Replace(message, lowertext(cprefs.speech_replace_to))
