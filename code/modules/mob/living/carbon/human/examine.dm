...
//this is very slightly better than it was because you can use it more places. still can't do \his[src] though.
	...
	//SKYRAT EDIT ADDITION BEGIN - EXAMINE RECORDS
	//var/datum/data/record/isinworld = find_record("name", perpname, GLOB.data_core.general)
	for(var/datum/antagonist/antag_datum in user?.mind?.antag_datums)
		if ((is_special_character(user)) /*&& isinworld*/ && (antag_datum.view_exploitables) && (src.has_dna()))
