...
/datum/species/proc/can_equip(obj/item/I, slot, disable_warning, mob/living/carbon/human/H, bypass_equip_delay_self = FALSE, ignore_equipped = FALSE)
	...
	// SKYRAT EDIT ADDITION START - MODULAR_SLOTS
	switch(modular_can_equip(H, I, slot, disable_warning, bypass_equip_delay_self))
		if(CAN_EQUIP_PASS)
			// Do nothing.
		if(CAN_EQUIP_TRUE)
			return TRUE
		if(CAN_EQUIP_FALSE)
			...
			CRASH("YOU RETURNED THE WRONG THING, CODER!! IF YOU ARE SEEING THIS, USE THE CAN_EQUIP DEFINES!!")
	// SKYRAT EDIT END
