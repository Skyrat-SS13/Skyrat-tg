	...
	if(pulling)
		if(isliving(pulling))
			...
			if(HAS_TRAIT(L, TRAIT_OVERSIZED) && !HAS_TRAIT(src, TRAIT_OVERSIZED) && !iscyborg(src))
				...
				return
			if(HAS_TRAIT(L, TRAIT_OVERWEIGHT) && !HAS_TRAIT(src, TRAIT_OVERSIZED) && !iscyborg(src))
				add_or_update_variable_movespeed_modifier(/datum/movespeed_modifier/bulky_drag, multiplicative_slowdown = PULL_OVERWEIGHT_SLOWDOWN)
				return
			//SKYRAT EDIT END
