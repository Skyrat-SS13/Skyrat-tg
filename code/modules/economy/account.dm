...
/datum/bank_account/proc/payday(amount_of_paychecks, free = FALSE)
	...
	// if(amount_of_paychecks == 1) // SKYRAT EDIT REMOVAL - Backgrounds - Why is this here anyway?
	money_to_transfer = round(clamp(money_to_transfer, 0, PAYCHECK_CREW * amount_of_paychecks) * BASE_PAYCHECK_MULTIPLIER * background_multiplier) // SKYRAT EDIT - Backgrounds - Proper ccaling paychecks, plus backgrounds modifiers. - ORIGINAL: <tab>money_to_transfer = clamp(money_to_transfer, 0, PAYCHECK_CREW) //We want to limit single, passive paychecks to regular crew income.
	if(free)
