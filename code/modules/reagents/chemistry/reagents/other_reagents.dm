...
/datum/reagent/determination/on_mob_end_metabolize(mob/living/carbon/M)
	if(significant)
		...
		for(var/thing in M.all_wounds)
			...
			stam_crash += (W.severity + 1) * 6 // spike of 3 stam damage per wound severity (moderate = 6, severe = 9, critical = 12) when the determination wears off if it was a combat rush
...
/datum/reagent/determination/on_mob_life(mob/living/carbon/M)
	...
	var/total_stamina_heal = 2
	for(var/thing in M.all_wounds)
		var/datum/wound/W = thing
		var/obj/item/bodypart/wounded_part = W.limb
		if(wounded_part)
			wounded_part.heal_damage(0.25, 0.25)
		total_stamina_heal += 0.5 // the more wounds, the more stamina regen
	M.adjustStaminaLoss(-total_stamina_heal*REM)
	..()
