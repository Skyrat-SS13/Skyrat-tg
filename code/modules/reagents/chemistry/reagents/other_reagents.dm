...
/datum/reagent/hauntium/expose_obj(obj/exposed_obj, volume) //gives 15 seconds of haunting effect for every unit of it that touches an object
	. = ..()
	if(HAS_TRAIT_FROM(exposed_obj, TRAIT_HAUNTED, HAUNTIUM_TRAIT_REAGENT))
		return
	exposed_obj.make_haunted(HAUNTIUM_TRAIT_REAGENT, "#f8f8ff")
	addtimer(CALLBACK(exposed_obj, TYPE_PROC_REF(/atom/movable/, remove_haunted), HAUNTIUM_TRAIT_REAGENT), volume * 20 SECONDS)
