...
/datum/round_event/shuttle_loan/announce(fake)
	...
	switch(dispatch_type)
		if(HIJACK_SYNDIE)
			priority_announce("Cargo: Rival corporate agents are trying to infiltrate your station. If you let them hijack your cargo shuttle, you'll save us a headache.","HighComm Counterintelligence")
		if(RUSKY_PARTY)
			priority_announce("Cargo: A group of angry Russians want to have a party. Can you send them your cargo shuttle then make them disappear?","HighComm Russian Outreach Program")
		if(SPIDER_GIFT)
			priority_announce("Cargo: The Spider Clan has sent us a mysterious gift. Can we ship it to you to see what's inside?","HighComm Diplomatic Corps")
		if(DEPARTMENT_RESUPPLY)
			priority_announce("Cargo: Seems we've ordered doubles of our department resupply packages this month. Can we send them to you?","HighComm Supply Department")
			thanks_msg = "The cargo shuttle should return in five minutes."
			bonus_points = 0
		if(ANTIDOTE_NEEDED)
			priority_announce("Cargo: Your station has been chosen for an epidemiological research project. Send us your cargo shuttle to receive your research samples.", "HighComm Research Initiatives")
		if(PIZZA_DELIVERY)
			priority_announce("Cargo: It looks like a neighbouring station accidentally delivered their pizza to you instead.", "HighComm Spacepizza Division")
			thanks_msg = "The cargo shuttle should return in five minutes."
			bonus_points = 0
		if(ITS_HIP_TO)
			priority_announce("Cargo: One of our freighters carrying a bee shipment has been attacked by eco-terrorists. Can you clean up the mess for us?", "HighComm Janitorial Division")
			bonus_points = 20000 //Toxin bees can be unbeelievably lethal
		if(MY_GOD_JC)
			priority_announce("Cargo: We have discovered an active Syndicate bomb near our VIP shuttle's fuel lines. If you feel up to the task, we will pay you for defusing it.", "HighComm Security Division")
...
/datum/round_event/shuttle_loan/proc/loan_shuttle()
	priority_announce(thanks_msg, "Cargo shuttle commandeered by HighComm.")
	...
	switch(dispatch_type)
		if(HIJACK_SYNDIE)
			SSshuttle.centcom_message += "Rival corporate hijack team incoming."
