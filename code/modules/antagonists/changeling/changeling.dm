...
/datum/antagonist/changeling/forge_objectives()
	...
	if(prob(60))
		...
		steal_objective.find_target(blacklist = minimum_opt_in_level(level = YES_TEMP)) //SKYRAT EDIT - ANTAG OPT IN
	...
	if(active_ais.len && prob(100/GLOB.joined_player_list.len))
		...
		destroy_objective.find_target(blacklist = minimum_opt_in_level(level = YES_KILL)) //SKYRAT EDIT - ANTAG OPT IN
		objectives += destroy_objective
	else
		if(prob(70))
			var/datum/objective/assassinate/kill_objective = new
			kill_objective.owner = owner
			kill_objective.find_target(blacklist = minimum_opt_in_level(level = YES_KILL)) //SKYRAT EDIT - ANTAG OPT IN
			objectives += kill_objective
		else
			var/datum/objective/maroon/maroon_objective = new
			maroon_objective.owner = owner
			maroon_objective.find_target(blacklist = minimum_opt_in_level(level = YES_TEMP)) //SKYRAT EDIT - ANTAG OPT IN
	...
	if (!(locate(/datum/objective/escape) in objectives) && escape_objective_possible)
		if(prob(50))
			...
			var/datum/objective/escape/escape_with_identity/identity_theft = new
			identity_theft.owner = owner
			identity_theft.find_target(blacklist = minimum_opt_in_level(level = YES_TEMP)) //SKYRAT EDIT - ANTAG OPT IN
			objectives += identity_theft
		escape_objective_possible = FALSE
