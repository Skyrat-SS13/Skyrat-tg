	...
	switch(action)
		if("PRG_speak")
			if(!channel || isnull(active_channel))
				return
			var/message = reject_bad_message(params["message"]) //SKYRAT EDIT CHANGE. ORIGINAL: var/message = reject_bad_text(params["message"])
			if(!message)
				return
		...
		if("PRG_leavechannel")
			if(channel)
				...
				return TRUE
		if("PRG_newchannel")
			var/channel_title = reject_bad_chatname(params["new_channel_name"]) //SKYRAT EDIT CHANGE. ORIGINAL: var/channel_title = reject_bad_text(params["new_channel_name"])
			if(!channel_title)
				return
		...
		if("PRG_toggleadmin")
			...
			if(can_run(user, TRUE, ACCESS_NETWORK))
				...
				return TRUE
		if("PRG_changename")
			var/newname = reject_bad_username(params["new_name"]) //SKYRAT EDIT CHANGE. ORIGINAL: var/newname = sanitize(params["new_name"])
		...
		if("PRG_renamechannel")
			if(!authed)
				return
			var/newname = reject_bad_chatname(params["new_name"]) //SKYRAT EDIT CHANGE. ORIGINAL: var/newname = reject_bad_text(params["new_name"])
			if(!newname || !channel)
				return
