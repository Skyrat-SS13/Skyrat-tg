...
/obj/item/stack/proc/make_item(mob/builder, datum/stack_recipe/recipe, multiplier)
	...
	if(recipe.max_res_amount > 1) // Is it a stack?
		...
		if(!isturf(covered_turf))
			return
		var/turf/turf_we_just_placed = covered_turf.PlaceOnTop(recipe.result_type, flags = CHANGETURF_INHERIT_AIR)
		...
		if(recipe.applies_mats && LAZYLEN(mats_per_unit))
			turf_we_just_placed.set_custom_materials(mats_per_unit, recipe.req_amount / recipe.res_amount)
