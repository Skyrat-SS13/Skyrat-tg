...
/obj/structure/girder/attackby(obj/item/W, mob/user, params)
	var/skill_modifier = crafter_mob.mind.get_skill_modifier(/datum/skill/construction, SKILL_SPEED_MODIFIER) //SKYRAT EDIT
	...
	if(istype(W, /obj/item/gun/energy/plasmacutter))
		...
		if(istype(W, /obj/item/stack/rods))
			...
			if(state == GIRDER_DISPLACED)
				...
				if(do_after(user, 20 * skill_modifier, target = src)) //SKYRAT EDIT
					if(rod.get_amount() < amount)
						return
					rod.use(amount)
				...
				if(do_after(user, 40 * skill_modifier, target = src)) //SKYRAT EDIT
					if(rod.get_amount() < amount)
						return
					rod.use(amount)
